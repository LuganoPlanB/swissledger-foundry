...
pub fn tempo_precompiles(cfg: &CfgEnv<TempoHardfork>) -> PrecompilesMap {
    let spec = if cfg.spec.is_t1c() { cfg.spec.into() } else { SpecId::PRAGUE };
...
mod tests {
    ...
    fn test_precompile_static_call() {
        ...
        let call_static = |calldata: Bytes| {
            ...
            db.insert_account_info(
                token_address,
                AccountInfo { code: Some(Bytecode::new_raw(bytes!("0xEF"))), ..Default::default() },
            );
            let mut evm = EthEvmFactory::default().create_evm(db, EvmEnv::default());
            let block = evm.block.clone();
            let evm_internals = EvmInternals::new(evm.journal_mut(), &block, &cfg, &tx);
        ...
        let result = call_static(Bytes::from(
            ITIP20::transferCall { to: Address::random(), amount: U256::from(100) }.abi_encode(),
        ));
        ...
        let result = call_static(Bytes::from(
            ITIP20::approveCall { spender: Address::random(), amount: U256::from(100) }
                .abi_encode(),
        ));
        let output = result.expect("expected Ok");
        assert!(output.is_revert());
        ...
        let result = call_static(Bytes::from(
            ITIP20::balanceOfCall { account: Address::random() }.abi_encode(),
        ));
        let output = result.expect("expected Ok");
        assert!(!output.is_revert(), "view function should not revert in static context");
    }
    ...
    fn test_early_return_revert_reports_gas_used() {
        ...
        let result = AlloyEvmPrecompile::call(&precompile, input);
        let output = result.expect("expected Ok");
        assert!(output.status.is_revert(), "uninitialized token should revert");
    ...
    fn test_invalid_calldata_hardfork_behavior() {
        ...
        assert!(output.is_halt(), "T0: expected halted output for invalid calldata");
    }
    ...
    fn test_precompile_state_gas_zero_pre_t4() {
        ...
        let result = call_with_spec(
            ITIP20::balanceOfCall::new((Address::ZERO,)).abi_encode().into(),
            TempoHardfork::T2,
        )
    ...
    fn test_t4_state_gas_only_includes_state_creating_ops() {
        ...
        let calldata: Bytes = ITIP20::balanceOfCall { account: sender }.abi_encode().into();
        let block = evm.block.clone();
        let tx = TxEnv::default();
        let evm_internals = EvmInternals::new(evm.journal_mut(), &block, &cfg, &tx);
        let input = PrecompileInput {
        ...
        let calldata: Bytes =
            ITIP20::transferCall { to: recipient, amount: U256::from(100) }.abi_encode().into();
        let block = evm.block.clone();
        let tx = TxEnv::default();
        let evm_internals = EvmInternals::new(evm.journal_mut(), &block, &cfg, &tx);
        let input = PrecompileInput {
    ...
    fn test_precompile_gas_refund_in_reservoir_t4() {
        ...
        let calldata: Bytes =
            ITIP20::transferCall { to: recipient, amount: U256::from(1000) }.abi_encode().into();
    ...
    fn test_dispatch_call_applies_hardfork_selector_gates() -> eyre::Result<()> {
        ...
        assert_eq!(decoded.selector.as_slice(), &ISelectorGatedTest::t2AddedCall::SELECTOR);
        ...
        assert_eq!(decoded.selector.as_slice(), &ISelectorGatedTest::t3RemovedCall::SELECTOR);

        // preT2: gated selectors must return `UnknownFunctionSelector` even for selector-only calldata.
        let malformed_added =
            call_with_spec(TempoHardfork::T1, &ISelectorGatedTest::t2AddedCall::SELECTOR)?;
    ...
    fn test_extend_tempo_precompiles_registers_precompiles() {
        ...
        assert!(validator_v2_precompile.is_some(), "ValidatorConfigV2 should be registered");
        ...
        assert!(sig_verifier_precompile.is_some(), "SignatureVerifier should be registered at T3");
    ...
    fn test_p256verify_availability_across_t1c_boundary() {
        ...
        for spec in [
            ...
            assert!(!has_p256(spec), "P256VERIFY should NOT be available at {spec:?} (pre-T1C)");
        }

        // T1C+ hardforks should use Osaka precompiles (P256VERIFY available)
        for spec in [TempoHardfork::T1C, TempoHardfork::T2] {
            assert!(has_p256(spec), "P256VERIFY should be available at {spec:?} (T1C+)");
        }
    }
}
