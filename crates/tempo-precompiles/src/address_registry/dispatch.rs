...
impl Precompile for AddressRegistry {
    fn call(&mut self, calldata: &[u8], msg_sender: Address) -> PrecompileResult {
        ...
        dispatch_call(calldata, &[], IAddressRegistryCalls::abi_decode, |call| match call {
            // Registration
            IAddressRegistryCalls::registerVirtualMaster(call) => {
                mutate(call, msg_sender, |s, c| self.register_virtual_master(s, c))
            }
            // View functions
            IAddressRegistryCalls::getMaster(call) => {
...
mod tests {
    ...
    fn test_get_master_precompile() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let call = IAddressRegistry::getMasterCall { masterId: Default::default() };
    ...
    fn test_is_virtual_address_precompile() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            // Non-virtual
            let call = IAddressRegistry::isVirtualAddressCall { addr: Address::random() };
            let result = registry.call(&call.abi_encode(), Address::ZERO)?;
            ...
            let call = IAddressRegistry::isVirtualAddressCall { addr: Address::from(bytes) };
            let result = registry.call(&call.abi_encode(), Address::ZERO)?;
            assert!(bool::abi_decode(&result.bytes).unwrap());
