...
impl TIP20Token {
    ...
    pub fn change_transfer_policy_id(
        ...
        if !TIP403Registry::new()
            .policy_exists(ITIP403Registry::policyExistsCall { policyId: call.newPolicyId })?
        {
    ...
    pub fn mint(&mut self, msg_sender: Address, call: ITIP20::mintCall) -> Result<()> {
        ...
        self.emit_event(TIP20Event::Mint(ITIP20::Mint { to: call.to, amount: call.amount }))?;
    ...
    pub fn burn(&mut self, msg_sender: Address, call: ITIP20::burnCall) -> Result<()> {
        ...
        self.emit_event(TIP20Event::Burn(ITIP20::Burn { from: msg_sender, amount: call.amount }))
    }
    ...
    pub fn permit(&mut self, call: ITIP20::permitCall) -> Result<()> {
        ...
        self.permit_nonces[call.owner].write(
            nonce.checked_add(U256::from(1)).ok_or(TempoPrecompileError::under_overflow())?,
        )?;
...
impl Recipient {
    ...
    pub(crate) fn resolve(addr: Address) -> Result<Self> {
        ...
        Ok(if effective == addr {
            ...
            Self { target: effective, virtual_addr: Some(addr) }
        })
    }
    ...
    pub(crate) fn build_virtual_transfer_event(&self, amount: U256) -> Option<TIP20Event> {
        self.virtual_addr.map(|virtual_addr| {
            TIP20Event::Transfer(ITIP20::Transfer { from: virtual_addr, to: self.target, amount })
        })
    }
}
...
pub(crate) mod tests {
    ...
    fn test_burn_with_memo() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            token.assert_emitted_events(vec![
                ...
                TIP20Event::Burn(ITIP20::Burn { from: admin, amount }),
            ]);
    ...
    fn test_transfer_from_with_memo_from_address() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            token.assert_emitted_events(vec![
                TIP20Event::Transfer(ITIP20::Transfer { from: owner, to, amount }),
                TIP20Event::TransferWithMemo(ITIP20::TransferWithMemo {
                    from: owner,
                    to,
                    amount,
    ...
    fn test_transfer_fee_pre_tx_insufficient_balance() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            let mut token = TIP20Setup::create("Test", "TST", admin).with_issuer(admin).apply()?;
    ...
    fn test_transfer_fee_post_tx() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(token.get_balance(TIP_FEE_MANAGER_ADDRESS)?, initial_fee - refund_amount);
            assert_eq!(
    ...
    fn test_update_quote_token() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            token.set_next_quote_token(
                admin,
                ITIP20::setNextQuoteTokenCall { newQuoteToken: new_quote_token_address },
            )?;
    ...
    fn test_finalize_quote_token_update_detects_loop() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let token_a =
                TIP20Setup::create("Token A", "TKA", admin).quote_token(token_b.address).apply()?;

            // Now try to set token_a as the next quote token for token_b (would create A -> B -> A loop)
            token_b.set_next_quote_token(
                admin,
                ITIP20::setNextQuoteTokenCall { newQuoteToken: token_a.address },
            )?;
    ...
    fn test_arbitrary_currency() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            for _ in 0..50 {
                ...
                let token = TIP20Setup::create("Test", "TST", admin).currency(&currency).apply()?;
    ...
    fn test_new_invalid_quote_token() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let token = TIP20Setup::create("Token", "T", admin).currency(&currency).apply()?;
    ...
    fn test_new_valid_quote_token() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let token_1 =
                TIP20Setup::create("USD Token", "USDT", admin).currency(currency_1).apply()?;
    ...
    fn test_update_quote_token_invalid_token() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let token_1 =
                TIP20Setup::create("Token 1", "TK1", admin).currency(&currency).apply()?;
            ...
            let result = usd_token.set_next_quote_token(
                admin,
                ITIP20::setNextQuoteTokenCall { newQuoteToken: token_1.address },
            );
    ...
    fn test_unable_to_burn_blocked_from_protected_address() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let balance =
                token.balance_of(ITIP20::balanceOfCall { account: STABLECOIN_DEX_ADDRESS })?;
            assert_eq!(balance, amount);
    ...
    fn test_transfer_invalid_recipient() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = token.transfer(admin, ITIP20::transferCall { to: Address::ZERO, amount });
            assert!(result.is_err_and(|err| err.to_string().contains("InvalidRecipient")));

            let result = token.transfer_from(
                bob,
                ITIP20::transferFromCall { from: admin, to: Address::ZERO, amount },
            );
            assert!(result.is_err_and(|err| err.to_string().contains("InvalidRecipient")));
    ...
    fn test_set_next_quote_token_rejects_path_usd() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = path_usd.set_next_quote_token(
                admin,
                ITIP20::setNextQuoteTokenCall { newQuoteToken: other_token.address },
            );
            assert!(matches!(
                result,
                Err(TempoPrecompileError::TIP20(TIP20Error::InvalidQuoteToken(_)))
    ...
    fn test_non_path_usd_cycle_detection() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let token_a =
                TIP20Setup::create("TokenA", "TKNA", admin).quote_token(token_b.address).apply()?;
        ...
        for hardfork in [TempoHardfork::T2, TempoHardfork::T3] {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                token.mint_with_memo(
                    admin,
                    ITIP20::mintWithMemoCall { to: virtual_addr, amount, memo: FixedBytes::ZERO },
                )?;
    ...
    fn test_transfer_from_to_virtual_address_credits_master() -> eyre::Result<()> {
        ...
        for hardfork in [TempoHardfork::T2, TempoHardfork::T3] {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                token.transfer_from(
                    spender,
                    ITIP20::transferFromCall { from: owner, to: virtual_addr, amount },
                )?;
    ...
    mod permit_tests {
        ...
        fn sign_permit(
            ...
            let struct_hash = keccak256(
                (*PERMIT_TYPEHASH, signer.address(), spender, value, nonce, deadline).abi_encode(),
            );
        ...
        fn make_permit_call(
            ...
            let (v, r, s) =
                sign_permit(signer, "Test", token_address, spender, value, nonce, deadline);
            ITIP20::permitCall { owner: signer.address(), spender, value, deadline, v, r, s }
        }

        #[test]
        fn test_permit_happy_path() -> eyre::Result<()> {
            let PermitFixture { mut storage, admin, ref signer, spender } = PermitFixture::new();
        ...
        fn test_permit_nonce_tracking() -> eyre::Result<()> {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                for i in 0u64..3 {
                    ...
                    assert_eq!(token.nonces(ITIP20::noncesCall { owner })?, U256::from(i + 1));
                }
        ...
        fn test_permit_works_when_paused() -> eyre::Result<()> {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                assert_eq!(token.allowance(ITIP20::allowanceCall { owner, spender })?, value);
        ...
        fn test_permit_domain_separator() -> eyre::Result<()> {
            let PermitFixture { mut storage, admin, .. } = PermitFixture::new();
        ...
        fn test_permit_max_allowance() -> eyre::Result<()> {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                assert_eq!(token.allowance(ITIP20::allowanceCall { owner, spender })?, U256::MAX);
        ...
        fn test_permit_allowance_override() -> eyre::Result<()> {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                token.permit(call)?;
                assert_eq!(token.allowance(ITIP20::allowanceCall { owner, spender })?, U256::ZERO);
        ...
        fn test_permit_invalid_v_values() -> eyre::Result<()> {
            let PermitFixture { mut storage, admin, spender, .. } = PermitFixture::new();
    ...
    fn test_burn_blocked_rejects_when_paused_on_t3() -> eyre::Result<()> {
        ...
        for hardfork in [TempoHardfork::T2, TempoHardfork::T3] {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                let result =
                    token.burn_blocked(admin, ITIP20::burnBlockedCall { from: blocked, amount });
