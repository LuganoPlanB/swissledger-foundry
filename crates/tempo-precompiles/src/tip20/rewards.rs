...
impl TIP20Token {
    ...
    pub fn get_pending_rewards(&self, account: Address) -> Result<u128> {
        ...
        pending.try_into().map_err(|_| TempoPrecompileError::under_overflow())
    }
}
...
mod tests {
    ...
    fn test_distribute_reward() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            // Distribute rewards
            token
                .distribute_reward(admin, ITIP20::distributeRewardCall { amount: reward_amount })?;
            ...
            token.mint(admin, ITIP20::mintCall { to: admin, amount: U256::from(1) })?;
            let result =
        ...
        for hardfork in [TempoHardfork::T2, TempoHardfork::T3] {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                let result = token.set_reward_recipient(
                    alice,
                    ITIP20::setRewardRecipientCall { recipient: virtual_addr },
                );
