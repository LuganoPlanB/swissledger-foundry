    ...
    fn call(&mut self, calldata: &[u8], msg_sender: Address) -> PrecompileResult {
        ...
        dispatch_call(calldata, &[], IStablecoinDEXCalls::abi_decode, |call| match call {
            IStablecoinDEXCalls::place(call) => {
                mutate(call, msg_sender, |s, c| self.place(s, c.token, c.amount, c.isBid, c.tick))
            }
            ...
            IStablecoinDEXCalls::getOrder(call) => {
                view(call, |c| self.get_order(c.orderId).map(|order| order.into()))
            }
...
mod tests {
    ...
    fn test_place_call() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let call = IStablecoinDEX::placeCall { token, amount: 100u128, isBid: true, tick: 0 };
    ...
    fn test_withdraw_call() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let call = IStablecoinDEX::withdrawCall { token, amount: 100u128 };
            let calldata = call.abi_encode();
