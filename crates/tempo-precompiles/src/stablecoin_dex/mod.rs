...
impl StablecoinDEX {
    ...
    fn sub_balance(&mut self, user: Address, token: Address, amount: u128) -> Result<()> {
        ...
        self.set_balance(
            user,
            token,
            current.checked_sub(amount).ok_or(TempoPrecompileError::under_overflow())?,
        )
    }
    ...
    fn emit_order_filled(
        ...
        self.emit_event(StablecoinDEXEvents::OrderFilled(IStablecoinDEX::OrderFilled {
            orderId: order_id,
            maker,
            taker,
    ...
    pub fn create_pair(&mut self, base: Address) -> Result<B256> {
        ...
        self.emit_event(StablecoinDEXEvents::PairCreated(IStablecoinDEX::PairCreated {
            key: book_key,
            base,
            quote,
        }))?;
    ...
    pub fn place(
        ...
        self.emit_event(StablecoinDEXEvents::OrderPlaced(IStablecoinDEX::OrderPlaced {
            orderId: order_id,
            maker: sender,
            token,
            amount,
    ...
    fn commit_order_to_book(&mut self, mut order: Order) -> Result<()> {
        ...
        let mut level =
            self.books[order.book_key()].tick_level_handler(order.tick(), order.is_bid()).read()?;
        ...
        if prev_tail == 0 {
            ...
            if order.is_bid() {
                if order.tick() > orderbook.best_bid_tick {
                    self.books[order.book_key()].best_bid_tick.write(order.tick())?;
                }
            } else if order.tick() < orderbook.best_ask_tick {
                self.books[order.book_key()].best_ask_tick.write(order.tick())?;
            }
        } else {
    ...
    fn partial_fill_order(
        ...
        self.orders[order.order_id()].remaining.write(new_remaining)?;
        ...
        let amount_out = if order.is_bid() { quote_amount } else { fill_amount };
    ...
    fn fill_order(
        ...
        let next_tick_info = if order.next() == 0 {
            self.books[book_key].tick_level_handler_mut(order.tick(), order.is_bid()).delete()?;
            ...
            if !has_liquidity {
                ...
                let new_level =
                    self.books[book_key].tick_level_handler(tick, order.is_bid()).read()?;
    ...
    fn get_best_price_level(&mut self, book_key: B256, is_bid: bool) -> Result<TickLevel> {
        ...
        self.books[book_key].tick_level_handler(current_tick, is_bid).read()
    }
    ...
    fn cancel_active_order(&mut self, order: Order) -> Result<()> {
        ...
        self.emit_event(StablecoinDEXEvents::OrderCancelled(IStablecoinDEX::OrderCancelled {
    ...
    fn is_maker_authorized(&self, order: &Order) -> Result<bool> {
        ...
        let (token_in, token_out) =
            if order.is_bid() { (book.quote, book.base) } else { (book.base, book.quote) };
    ...
    fn quote_exact_out(&self, book_key: B256, amount_out: u128, is_bid: bool) -> Result<u128> {
        ...
        while remaining_out > 0 {
            let level = self.books[book_key].tick_level_handler(current_tick, is_bid).read()?;
...
mod tests {
    ...
    fn test_calculate_quote_amount_rounding() -> eyre::Result<()> {
        ...
        assert_eq!(result_floor, 100, "Expected 100 (rounded down from 100.001)");
        ...
        assert_eq!(result2_floor, 999, "Expected 999 (rounded down from 999.04995)");

        // Same inputs with ceiling should round up to 1000
        let result2_ceil = base_to_quote(amount2, tick2, RoundingDirection::Up).unwrap();
        assert_eq!(result2_ceil, 1000, "Expected 1000 (rounded up from 999.04995)");
    ...
    fn test_cancellation_refund_equals_escrow_for_bid_orders() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let base = TIP20Setup::create("BASE", "BASE", admin).with_issuer(admin).apply()?;
            let base_token = base.address();
            let quote_token = base.quote_token()?;
            ...
            assert_eq!(alice_balance_after_place, 0, "All quote tokens should be escrowed");
    ...
    fn test_place_flip_order_below_minimum_amount() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result =
                exchange.place_flip(alice, base_token, below_minimum, true, tick, flip_tick, false);
            assert_eq!(
                result,
                Err(StablecoinDEXError::below_minimum_order_size(below_minimum).into())
            );
    ...
    fn test_place_flip_order() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let order_id = exchange
                .place_flip(alice, base_token, min_order_amount, true, tick, flip_tick, false)
    ...
    fn test_withdraw_insufficient_balance() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(result, Err(StablecoinDEXError::insufficient_balance().into()));
    ...
    fn test_pair_created() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            exchange.assert_emitted_events(vec![StablecoinDEXEvents::PairCreated(
                IStablecoinDEX::PairCreated { key, base: base_token, quote: quote_token },
            )]);
    ...
    fn test_pair_already_created() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(result, Err(StablecoinDEXError::pair_already_exists().into()));
    ...
    fn test_create_pair_rejects_non_tip20_base() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(matches!(
                result,
                Err(TempoPrecompileError::StablecoinDEX(StablecoinDEXError::InvalidBaseToken(_)))
            ));
    ...
    fn test_find_trade_path_rejects_non_tip20() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(
                matches!(
                    result,
                    Err(TempoPrecompileError::StablecoinDEX(StablecoinDEXError::InvalidToken(_)))
                ),
    ...
    fn test_place_order_immediately_active() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(orderbook.best_bid_tick, tick, "Best bid tick should be updated");
    ...
    fn test_place_flip_order_immediately_active() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(stored_order.flip_tick(), flip_tick, "Flip tick should match");
    ...
    fn test_place_post() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let book_key = compute_book_key(base_token, quote_token);
            let level = exchange.books[book_key].tick_level_handler(tick, true).read()?;
            assert_eq!(level.head, order_id);
            assert_eq!(level.tail, order_id);
            assert_eq!(level.total_liquidity, min_order_amount);
    ...
    fn test_blacklisted_user_cannot_use_internal_balance() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let mut base = TIP20Setup::create("BASE", "BASE", admin).with_issuer(admin).apply()?;
            let base_address = base.address();
        ...
        for spec in [TempoHardfork::T0, TempoHardfork::T1C, TempoHardfork::T2] {
            ...
            let (order_id, base_token, invalid_policy_id) =
                StorageCtx::enter(&mut storage, || {
                    ...
                    base.change_transfer_policy_id(
                        admin,
                        ITIP20::changeTransferPolicyIdCall { newPolicyId: invalid_policy_id },
                    )?;
    ...
    fn test_compound_policy_non_escrow_token_direction() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            quote.change_transfer_policy_id(
                admin,
                ITIP20::changeTransferPolicyIdCall { newPolicyId: compound_id },
            )?;
    ...
    fn test_get_order_validates_maker_and_order_id() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(result.is_err());
            assert_eq!(result.unwrap_err(), StablecoinDEXError::order_does_not_exist().into());
    ...
    fn test_flip_order_fill_ignores_business_logic_error() -> eyre::Result<()> {
        ...
        for spec in [TempoHardfork::T1, TempoHardfork::T1A, TempoHardfork::T2] {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                let level = exchange.books[book_key].tick_level_handler(flip_tick, false).read()?;
                assert_eq!(level.total_liquidity, 0, "[{spec:?}] No flipped order should exist");
        ...
        for spec in [TempoHardfork::T1, TempoHardfork::T1A, TempoHardfork::T2] {
            ...
            StorageCtx::enter(&mut storage, || {
                ...
                if spec.is_t1a() {
                    ...
                    assert!(result.is_err(), "Swap should revert when flip hits a system error");
                    assert!(
    ...
    fn test_orderbook_invariants_after_all_orders_filled() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(bid_level.total_liquidity, 0, "bid level liquidity must be 0 after drain");
            ...
            assert_eq!(ask_level.total_liquidity, 0, "ask level liquidity must be 0 after drain");
