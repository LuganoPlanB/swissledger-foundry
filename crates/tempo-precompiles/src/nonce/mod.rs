...
mod tests {
    ...
    fn test_increment_nonce() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            mgr.assert_emitted_events(vec![
                INonce::NonceIncremented { account, nonceKey: nonce_key, newNonce: 1 },
                INonce::NonceIncremented { account, nonceKey: nonce_key, newNonce: 2 },
            ]);
    ...
    fn test_ring_buffer_pointer_wraps_at_capacity() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            mgr.expiring_nonce_ring_ptr.write(EXPIRING_NONCE_SET_CAPACITY - 1)?;
