...
impl<'a> EvmPrecompileStorageProvider<'a> {
    ...
    pub fn new_max_gas(internals: EvmInternals<'a>, cfg: &CfgEnv<TempoHardfork>) -> Self {
        Self::new(internals, u64::MAX, 0, cfg.spec, false, cfg.gas_params.clone())
    }
    ...
    pub fn new_with_gas_limit(
        ...
        Self::new(internals, gas_limit, reservoir, cfg.spec, false, cfg.gas_params.clone())
    }
...
impl<'a> PrecompileStorageProvider for EvmPrecompileStorageProvider<'a> {
    ...
    fn with_account_info(
        ...
        let mut account = self.internals.load_account_mut_skip_cold_load(address, false)?;
    ...
    fn sstore(
        ...
        let result = self.internals.load_account_mut(address)?.sstore(key, value, false)?;
