...
enum Action {
    ...
    CreateToken { name: &'static str, symbol: &'static str, currency: String },
...
impl TIP20Setup {
    ...
    pub fn create(name: &'static str, symbol: &'static str, admin: Address) -> Self {
        Self {
            action: Action::CreateToken { name, symbol, currency: "USD".into() },
            admin: Some(admin),
            ..Default::default()
        }
    }
...
pub fn register_virtual_master(registry: &mut AddressRegistry) -> Result<(MasterId, Address)> {
    let master_id = registry.register_virtual_master(
        VIRTUAL_MASTER,
        IAddressRegistry::registerVirtualMasterCall { salt: VIRTUAL_SALT.into() },
    )?;
