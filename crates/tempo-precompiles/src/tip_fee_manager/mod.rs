...
impl TipFeeManager {
    ...
    pub fn get_validator_token(&self, beneficiary: Address) -> Result<Address> {
        ...
        if token.is_zero() { Ok(DEFAULT_FEE_TOKEN) } else { Ok(token) }
    }
    ...
    pub fn distribute_fees(&mut self, validator: Address, token: Address) -> Result<()> {
        ...
        tip20_token.transfer(self.address, ITIP20::transferCall { to: validator, amount })?;
...
mod tests {
    ...
    fn test_set_user_token_noop_when_unchanged_pre_t3() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(event_count, 2, "pre-T3: event emitted even when token unchanged");
    ...
    fn test_collect_fee_pre_tx_different_tokens() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            fee_manager.set_validator_token(
                validator,
                IFeeManager::setValidatorTokenCall { token: validator_token.address() },
                Address::random(),
            )?;
            ...
            assert_eq!(pool.reserve_user_token, 10000, "Reserves unchanged in pre_tx");
            assert_eq!(pool.reserve_validator_token, 10000, "Reserves unchanged in pre_tx");
    ...
    fn test_collect_fee_pre_tx_skip_liquidity_check() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let validator_token =
                TIP20Setup::create("ValidatorToken", "VTK", admin).with_issuer(admin).apply()?;
            ...
            assert!(result.is_err(), "Should fail without liquidity, got: {result:?}");
