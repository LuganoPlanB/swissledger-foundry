...
impl TipFeeManager {
    ...
    pub fn check_sufficient_liquidity(&mut self, pool_id: B256, max_amount: U256) -> Result<u128> {
        ...
        amount_out_needed.try_into().map_err(|_| TempoPrecompileError::under_overflow())
    }
    ...
    pub fn execute_fee_swap(
        ...
        // Update reserves
        let amount_in_u128: u128 =
            amount_in.try_into().map_err(|_| TempoPrecompileError::under_overflow())?;
        let amount_out_u128: u128 =
            amount_out.try_into().map_err(|_| TempoPrecompileError::under_overflow())?;
...
mod tests {
    ...
    fn test_mint_identical_addresses() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = amm.mint(admin, token.address(), token.address(), U256::from(1000), admin);
            assert!(matches!(
                result,
    ...
    fn test_mint_rejects_non_usd_user_token() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result =
                amm.mint(admin, eur_token.address(), usd_token.address(), U256::from(1000), admin);
            assert!(matches!(
                result,
            ...
            let result =
                amm.mint(admin, usd_token.address(), eur_token.address(), U256::from(1000), admin);
            assert!(matches!(
                result,
                Err(TempoPrecompileError::TIP20(TIP20Error::InvalidCurrency(_)))
            ));
    ...
    fn test_execute_fee_swap_immediate() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(U256::from(pool.reserve_user_token), liquidity_amount + amount_in);
            assert_eq!(U256::from(pool.reserve_validator_token), liquidity_amount - expected_out);
    ...
    fn test_rebalance_swap() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(U256::from(pool.reserve_validator_token), liquidity + amount_in);
    ...
    fn test_mint_subsequent_deposit() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let second_liquidity =
                amm.mint(second_user, user_token, validator_token, second_amount, second_user)?;
            ...
            assert_eq!(total_supply_after_second, total_supply_after_first + second_liquidity);
    ...
    fn test_burn_insufficient_balance() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = amm.burn(other_user, user_token, validator_token, liquidity, other_user);
