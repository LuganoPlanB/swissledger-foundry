...
impl TIP403Registry {
    ...
    pub fn policy_data(
        ...
        if self.storage.spec().is_t2() {
            ...
            if !self.policy_exists(ITIP403Registry::policyExistsCall { policyId: call.policyId })? {
        ...
        Ok(ITIP403Registry::policyDataReturn { policyType: data.policy_type()?, admin: data.admin })
    }
    ...
    pub fn compound_policy_data(
        ...
        if !data.is_compound() {
            ...
            let err = if self
                .policy_exists(ITIP403Registry::policyExistsCall { policyId: call.policyId })?
            {
...
impl AuthRole {
    ...
    fn transfer_or(t2_variant: Self) -> Self {
        if StorageCtx.spec().is_t2() { t2_variant } else { Self::Transfer }
    }
...
mod tests {
    ...
    fn test_policy_exists() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            for policy_id in &created_policy_ids {
                assert!(
                    registry.policy_exists(ITIP403Registry::policyExistsCall {
                        policyId: *policy_id
                    })?
                );
            }
    ...
    fn test_create_compound_policy() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(
                registry
                    .policy_exists(ITIP403Registry::policyExistsCall { policyId: compound_id })?
            );

            // Verify policy type is COMPOUND
            let data =
                registry.policy_data(ITIP403Registry::policyDataCall { policyId: compound_id })?;
    ...
    fn test_create_policy_rejects_non_simple_policy_types() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
                ...
                [ITIP403Registry::PolicyType::COMPOUND, ITIP403Registry::PolicyType::__Invalid]
            {
                let result = registry.create_policy(
                    admin,
                    ITIP403Registry::createPolicyCall { admin, policyType: policy_type },
                );
                assert!(matches!(
                    result.unwrap_err(),
    ...
    fn test_pre_t1_create_policy_with_invalid_type_stores_255() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
                ...
                [ITIP403Registry::PolicyType::COMPOUND, ITIP403Registry::PolicyType::__Invalid]
            {
                ...
                assert!(
                    registry
                        .policy_exists(ITIP403Registry::policyExistsCall { policyId: policy_id })?
                );
    ...
    fn test_pre_t1_policy_data_reverts_for_any_policy_type_gte_2() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result =
                registry.policy_data(ITIP403Registry::policyDataCall { policyId: policy_id });
            assert!(result.is_err());
            assert_eq!(result.unwrap_err(), TempoPrecompileError::under_overflow());
    ...
    fn test_pre_t2_to_t2_migration_invalid_policy_still_fails() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result =
                registry.policy_data(ITIP403Registry::policyDataCall { policyId: policy_id });
            assert!(result.is_err());
            assert_eq!(result.unwrap_err(), TIP403RegistryError::invalid_policy_type().into());
    ...
    fn test_policy_exists_boundary_at_counter() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(
                !registry
                    .policy_exists(ITIP403Registry::policyExistsCall { policyId: counter + 1 })?
            );
    ...
    fn test_nonexistent_policy_behavior() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(
                registry.is_authorized_as(nonexistent_id, user, AuthRole::Transfer).unwrap_err(),
                TIP403RegistryError::policy_not_found().into()
            );
            Ok(())
        })
    ...
    fn test_modify_whitelist_rejects_virtual_address() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let policy_id = registry.create_policy(
                admin,
                ITIP403Registry::createPolicyCall { admin, policyType: PolicyType::WHITELIST },
            )?;
    ...
    fn test_modify_blacklist_rejects_virtual_address() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let policy_id = registry.create_policy(
                admin,
                ITIP403Registry::createPolicyCall { admin, policyType: PolicyType::BLACKLIST },
            )?;
