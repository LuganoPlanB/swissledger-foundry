...
impl Config {
    fn new(owner: Address, is_init: bool, init_at_height: u64) -> Self {
        Self { owner, is_init, init_at_height, migration_skipped_count: 0, v1_validator_count: 0 }
    }
...
impl ValidatorConfigV2 {
    ...
    fn require_unique_ingress(&self, ingress: &str) -> Result<B256> {
        ...
        if self.active_ingress_ips[ingress_hash].read()? {
            Err(ValidatorConfigV2Error::ingress_already_exists(ingress.to_string()))?
        }
        Ok(ingress_hash)
    }
    ...
    fn update_ingress_ip_tracking(&mut self, old_ingress: &str, new_ingress: &str) -> Result<()> {
        ...
        if old_ingress_hash != new_ingress_hash {
            if self.active_ingress_ips[new_ingress_hash].read()? {
                Err(ValidatorConfigV2Error::ingress_already_exists(new_ingress.to_string()))?
            }
    ...
    fn require_new_address(&self, addr: Address) -> Result<()> {
        ...
        let idx1 = self.address_to_index[addr].read()?;
        if idx1 != 0 && self.validators[(idx1 - 1) as usize].deactivated_at_height.read()? == 0 {
    ...
    pub fn add_validator(
        ...
        self.verify_validator_signature(
            SignatureKind::Add { fee_recipient: call.feeRecipient },
            &call.publicKey,
            &call.signature,
    ...
    pub fn deactivate_validator(
        ...
        self.config.read()?.require_owner_or_validator(sender, v.validator_address)?;

        self.active_ingress_ips[Self::ingress_key(&v.ingress)?].delete()?;

        let block_height = self.storage.block_number();
        self.validators[call.idx as usize].deactivated_at_height.write(block_height)?;
    ...
    pub fn migrate_validator(
        ...
        let skip = |s: &mut Self| {
            ...
            s.config.migration_skipped_count.write(skipped.saturating_add(1))
        };
        ...
        if addr_idx != 0
            && self.validators[(addr_idx - 1) as usize].deactivated_at_height.read()? == 0
        {
            Err(ValidatorConfigV2Error::address_already_has_validator())?
        }
    ...
    pub fn initialize_if_migrated(&mut self, sender: Address) -> Result<()> {
        ...
        self.emit_event(ValidatorConfigV2Event::Initialized(IValidatorConfigV2::Initialized {
            height,
        }))
    }
}
...
mod tests {
    ...
    fn test_add_validator() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let (pubkey, signature) = make_test_keypair_and_signature(
                ...
                "192.168.1.1:8000",
                "192.168.1.1",
                SignatureKind::Add { fee_recipient: validator },
            );
    ...
    fn test_add_validator_rejects_zero_pubkey() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(result, Err(ValidatorConfigV2Error::invalid_public_key().into()));
    ...
    fn test_add_validator_rejects_duplicate_address() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(result, Err(ValidatorConfigV2Error::address_already_has_validator().into()));
    ...
    fn test_add_validator_rejects_duplicate_pubkey() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let (pubkey, sig1) = make_test_keypair_and_signature(
                ...
                "192.168.1.1:8000",
                "192.168.1.1",
                SignatureKind::Add { fee_recipient: addr1 },
            );
            vc.storage.set_block_number(200);
            vc.add_validator(
                owner,
                make_add_call(addr1, pubkey, "192.168.1.1:8000", "192.168.1.1", addr1, sig1),
            )?;
            ...
            let (_, sig2) = make_test_keypair_and_signature(
                ...
                "192.168.1.2:8000",
                "192.168.1.2",
                SignatureKind::Add { fee_recipient: addr2 },
            );
            vc.storage.set_block_number(201);
            let result = vc.add_validator(
                owner,
                make_add_call(addr2, pubkey, "192.168.1.2:8000", "192.168.1.2", addr2, sig2),
            );
            assert_eq!(result, Err(ValidatorConfigV2Error::public_key_already_exists().into()));
    ...
    fn test_deactivate_validator() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            vc.deactivate_validator(owner, IValidatorConfigV2::deactivateValidatorCall { idx: 0 })?;
            ...
            assert_eq!(result, Err(ValidatorConfigV2Error::validator_already_deactivated().into()));
    ...
    fn test_deactivate_validator_dual_auth() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            vc.storage.set_block_number(301);
            vc.deactivate_validator(owner, IValidatorConfigV2::deactivateValidatorCall { idx: 1 })?;
    ...
    fn test_set_fee_recipient() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            vc.set_fee_recipient(
                owner,
                IValidatorConfigV2::setFeeRecipientCall { idx: 0, feeRecipient: fee_recipient_1 },
            )?;
            ...
            vc.set_fee_recipient(
                validator,
                IValidatorConfigV2::setFeeRecipientCall { idx: 0, feeRecipient: fee_recipient_2 },
            )?;
    ...
    fn test_transfer_ownership() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(result, Err(ValidatorConfigV2Error::not_initialized().into()));
            ...
            let result = vc.transfer_ownership(
                owner,
                IValidatorConfigV2::transferOwnershipCall { newOwner: Address::ZERO },
            );
            ...
            let result = vc.transfer_ownership(
                owner,
                IValidatorConfigV2::transferOwnershipCall { newOwner: Address::random() },
            );
            assert_eq!(result, Err(ValidatorConfigV2Error::unauthorized().into()));
    ...
    fn test_transfer_validator_ownership() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            vc.add_validator(
                owner,
                make_add_call(validator, pubkey, "192.168.1.1:8000", "192.168.1.1", validator, sig),
            )?;
            ...
            assert_eq!(result, Err(ValidatorConfigV2Error::validator_not_found().into()));
    ...
    fn test_egress_validates_ip_without_port() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result = vc.add_validator(
                owner,
                make_add_call(addr1, pubkey1, "192.168.1.1:8000", "192.168.1.1:9000", addr1, sig1),
            );
    ...
    fn test_migration_from_v1() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let result =
                v2.migrate_validator(owner, IValidatorConfigV2::migrateValidatorCall { idx: 0 });
            assert_eq!(result, Err(ValidatorConfigV2Error::already_initialized().into()));
    ...
    fn test_migration_out_of_order_fails() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(result, Err(ValidatorConfigV2Error::invalid_migration_index().into()));
    ...
    fn test_initialize_before_migration_complete_fails() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(result, Err(ValidatorConfigV2Error::migration_not_complete().into()));
    ...
    fn test_add_validator_reuses_deactivated_address() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            let (pubkey1, sig1) = make_test_keypair_and_signature(
                ...
                "192.168.1.1:8000",
                "192.168.1.1",
                SignatureKind::Add { fee_recipient: validator_addr },
            );
            vc.storage.set_block_number(200);
            vc.add_validator(
                owner,
    ...
    fn test_rotate_validator_to_different_ingress() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(vc.validator_by_address(validator)?.ingress, "192.168.1.1:8001");
    ...
    fn test_ipv6_canonical_representation() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert!(result.is_err(), "Different IPv6 notations of same IP should be rejected");
    ...
    fn test_add_validator_rejects_wrong_key_signature() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(result, Err(ValidatorConfigV2Error::invalid_signature().into()));
    ...
    fn test_add_validator_rejects_malformed_signature() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            assert_eq!(result, Err(ValidatorConfigV2Error::invalid_signature_format().into()));
    ...
    fn test_event_emission_owner_and_validator_actions() -> eyre::Result<()> {
        ...
        StorageCtx::enter(&mut storage, || {
            ...
            vc.assert_emitted_events(vec![ValidatorConfigV2Event::OwnershipTransferred(
                IValidatorConfigV2::OwnershipTransferred { oldOwner: owner, newOwner: new_owner },
            )]);
