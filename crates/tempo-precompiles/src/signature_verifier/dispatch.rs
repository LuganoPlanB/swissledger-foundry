...
impl Precompile for SignatureVerifier {
    fn call(&mut self, calldata: &[u8], _msg_sender: Address) -> PrecompileResult {
        ...
        if calldata.len() > MAX_CALLDATA_LEN {
            return Ok(self.storage.abi_revert(SignatureVerifierError::invalid_format()));
        }
        ...
        dispatch_call(calldata, &[], ISVCalls::abi_decode, |call| match call {
            ...
            ISVCalls::verify(call) => {
                view(call, |c| self.recover(c.hash, c.signature).map(|sig| sig == c.signer))
            }
        })
    }
}
